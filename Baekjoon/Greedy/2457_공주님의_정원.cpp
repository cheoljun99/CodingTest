...
int main() {
	...
	for(int i=0;i<N;i++){
		...
		if (vec[i].first > start && vec[i].first <= end) { 
			...
			if (end >= maxEnd) {
				//이부분이 없으면 시간초가
				// 아마도 end는 무조건 maxEnd보다 작아야하는데 이유는 maxEnd는 선택한 백터 즉 꽃의 지는 날짜의 갱신 전인데 이 갱신 전 날짜는
				// 현재의 end보다는 무조건 커야지 이후의 날짜 갱신에서 11월 30일 까지 계속해서 증가하여 꽃의 피는 날을 늘리수 가 있다.
				// 만약 end가 maxEnd보다 크게 될 경우가 있으면 이는 그리디의 성격과 안맞을 뿐만 아니라 꽃이 지는 날짜가 오히려 현재 보다 짧아지게 된다.
				break;
			}
